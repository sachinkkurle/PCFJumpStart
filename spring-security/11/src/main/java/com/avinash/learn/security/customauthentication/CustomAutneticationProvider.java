...
import com.avinash.learn.security.CustomAuthenticationToken;
...
public class CustomAutneticationProvider implements AuthenticationProvider {
	...
	public Authentication authenticate(Authentication arg0) throws AuthenticationException {
		...
		CustomAuthenticationToken tone = (CustomAuthenticationToken)arg0;
		AutoUser user = repo.findByUsername(tone.getName());
		if(user == null || 
				(!user.getPassword().equalsIgnoreCase(tone.getCredentials().toString())
				|| !tone.getMake().equalsIgnoreCase("subaru")						
						)
				
				) {
			throw new BadCredentialsException("bad credential");
		}
		return new CustomAuthenticationToken(user,user.getPassword(), user.getAuthorities(), tone.getMake());
	}
